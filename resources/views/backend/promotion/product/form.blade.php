		...
		<div class="row clearfix">
			...
			<div class="col-lg-12 col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading">จัดส่วนลดรายสินค้า</div>
					<div class="panel-body">
						<form action="@if(empty($sRow)) {{ action('Backend\Promotion\ProductController@store') }}@else {{action('Backend\Promotion\ProductController@update', $sRow->id )}}@endif" method="POST" autocomplete="off">
						{{ csrf_field() }}
									...
									<div class="form-group">
										...
										<div class="input-group">
											...
											@if($sProduct->count())
												...
												<option value="{{$r->pd_id}}" {{ (@$sRow->product_id==$r->pd_id)?'selected':'' }}>{{$r->name}}</option>
												@endforeach
											@endif
                                    		</select>
										</div>
