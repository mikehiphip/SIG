				...
				<div class="form-horizontal">
					<div class="form-group m-b-0 hidden-xs hidden-sm">
						...
						<div class="col-sm-4 align-right">
                            <a class="btn btn-info btn-sm" href="{{ action("Backend\Promotion\MatchController@create") }}">เพิ่มโปรโมชั่น</a>
						</div>
					</div>
				</div>
...
$(function() {
		...
		iDisplayLength: 25,
        ajax: {
			url: '{{ action("Backend\Promotion\MatchController@Datatable") }}',
			data: function ( d ) {
				d.Where={};
        ...
        columns: [
            ...
            {data: 'name', title :'ชื่อโปรโมชั่น', className: 'text-center'},
		...
		rowCallback: function(nRow, aData, dataIndex){
			$('td:last-child', nRow).html(''
				+ '<a href="{{ action("Backend\Promotion\MatchController@index") }}/'+aData['id']+'/edit'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> แก้ไข</a> '
				+ '<a data-url="{{ action("Backend\Promotion\MatchController@index") }}/'+aData['id']+'/" class="btn btn-xs btn-danger cDelete" ><i class="glyphicon glyphicon-remove"></i> ลบ</a>'
			).addClass('input');
		}
    });
